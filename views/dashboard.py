...
#
# Purpose:
# - 首頁儀表板 UI：交接摘要、保養摘要、低水位、本月用量與即時庫存圖表。
#
# Major Changes in This Revision:
# - 回復即時庫存 / 即時回用料庫存長條圖為接近原本固定寬度視覺。
# - 保留原本漸層長條顏色，不改成手機兩列式，也不縮小整體圖表。
# - 配合 service / repo 修正，首頁保養摘要不再顯示已軟刪除保養項目。
#
# Notes:
# - Flet 0.84。
# - 不使用 page.push_route()。
# - 本次不修改 reports.py、CSV/PDF 下載機制、Supabase schema 或 Nginx /exports/ 設定。
# =====================================================
